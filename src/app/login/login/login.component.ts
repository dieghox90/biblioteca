...
export class LoginComponent implements OnInit {
  ...
  login(): void {
    ...
    this.usuarioService.getUsuarioLogin(this.usuario.username).subscribe(u => {
      if (u == null) {
        Swal.fire('Error', 'No existe un usuario registrado con el correo ingresada', 'error');
        this.spinner.hide();
      } else {
  ...
  enviarCodigo(): void {
    ...
    if (this.EMAIL_REGEX.test(this.correo)) {
      ...
      this.usuarioService.enviarCorreoRecuperacion(this.correo).subscribe(res => {
        if (res == null) {
          this.toastr.error('No existe un usuario registrado con el correo ingresado', 'Error');
          this.cargando = false;
        } else {
